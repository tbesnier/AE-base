...
def vertex_quadrics(mesh):
    ...
    # Allocate quadrics
    v_quadrics = np.zeros((len(mesh.vertices), 4, 4,))

    # For each face...
    for f_idx in range(len(mesh.faces)):

        # Compute normalized plane equation for that face
        vert_idxs = mesh.faces[f_idx]
        verts = np.hstack((mesh.vertices[vert_idxs], np.array([1, 1, 1]).reshape(-1, 1)))
        ...
        for k in range(3):
            v_quadrics[mesh.faces[f_idx, k], :, :] += np.outer(eq, eq)

    return v_quadrics

def setup_deformation_transfer(source, target, use_normals=False):
    rows = np.zeros(3 * target.vertices.shape[0])
    cols = np.zeros(3 * target.vertices.shape[0])
    coeffs_v = np.zeros(3 * target.vertices.shape[0])
    coeffs_n = np.zeros(3 * target.vertices.shape[0])

    #nearest_faces, nearest_parts, nearest_vertices = source.compute_aabb_tree().nearest(target.vertices, True)
    nearest_faces = tri.proximity.closest_point(source, target.vertices)[2]
    nearest_vertices = tri.proximity.closest_point(source, target.vertices)[0]
    nearest_parts = np.where(np.sign(tri.proximity.signed_distance(source, target.vertices))>0, 0, 1)
    ...
    nearest_faces = np.array(nearest_faces).reshape(-1)
    nearest_parts = np.array(nearest_parts).reshape(-1)
    nearest_vertices = np.array(nearest_vertices).reshape(-1)

    for i in range(target.vertices.shape[0]):
        # Closest triangle index
        f_id = nearest_faces[i]
        # Closest triangle vertex ids
        nearest_f = source.faces[f_id]

        # Closest surface point
        nearest_v = nearest_vertices[3 * i:3 * i + 3]
        # Distance vector to the closest surface point
        dist_vec = target.vertices[i] - nearest_v
        ...
        if n_id == 0:
            ...
            A = np.vstack((source.vertices[nearest_f])).T
            
            coeffs_v[3 * i:3 * i + 3] = np.linalg.lstsq(A, nearest_v)[0]
        elif n_id > 0 and n_id <= 3:
            # Closest surface point on edge
            A = np.vstack((source.vertices[nearest_f[n_id - 1]], source.vertices[nearest_f[n_id % 3]])).T
            tmp_coeffs = np.linalg.lstsq(A, target.vertices[i])[0]
    ...
    matrix = sp.csc_matrix((coeffs_v, (rows, cols)), shape=(target.vertices.shape[0], source.vertices.shape[0]))
    return matrix
...
def qslim_decimator_transformer(mesh, factor=None, n_verts_desired=None):
    ...
    if n_verts_desired is None:
        n_verts_desired = math.ceil(len(mesh.vertices) * factor)
    ...
    vert_adj = get_vertices_per_edge(mesh.vertices, mesh.faces)
    # vert_adj = sp.lil_matrix((len(mesh.v), len(mesh.v)))
    # for f_idx in range(len(mesh.f)):
    #     vert_adj[mesh.f[f_idx], mesh.f[f_idx]] = 1

    vert_adj = sp.csc_matrix((vert_adj[:, 0] * 0 + 1, (vert_adj[:, 0], vert_adj[:, 1])), shape=(len(mesh.vertices), len(mesh.vertices)))
    ...
    for k in range(vert_adj.nnz):
        ...
        cost = collapse_cost(Qv, r, c, mesh.vertices)['collapse_cost']
        heapq.heappush(queue, (cost, (r, c)))

    # decimate
    collapse_list = []
    nverts_total = len(mesh.vertices)
    ...
    while nverts_total > n_verts_desired:
        ...
        cost = collapse_cost(Qv, r, c, mesh.vertices)
    ...
    new_faces, mtx = _get_sparse_transform(faces, len(mesh.vertices))
...
def generate_transform_matrices(mesh, factors):
    ...
    F = []
    ##
    A.append(get_vert_connectivity(mesh.vertices, mesh.faces))
    M.append(mesh)
    ...
    for factor in factors:
        ...
        F.append(ds_f)
        ##
        new_mesh_v = ds_D.dot(M[-1].vertices)     
        new_mesh = tri.Trimesh(vertices=new_mesh_v,faces=ds_f)
        M.append(new_mesh)
        A.append(get_vert_connectivity(new_mesh.vertices, new_mesh.faces))
...
def generate_transform_matrices_given_downsamples(mesh, downsample_directory, num_downsamples=4):
    ...
    for i in range(1,num_downsamples+1):
        cur_M = tri.load_mesh(filename=os.path.join(downsample_directory,'template_d{0}.obj'.format(i)))
        cur_D = np.zeros((cur_M.vertices.shape[0], M[-1].vertices.shape[0]))
        kd = spatial.KDTree(np.array(M[-1].vertices))
        for vi in range(cur_M.vertices.shape[0]):
            _, u = kd.query(cur_M.vertices[vi])
            cur_D[vi,u] = 1.0
        M.append(cur_M)
        D.append(sp.csr_matrix(cur_D))
        F.append(cur_M.faces)
        A.append(get_vert_connectivity(cur_M.vertices,cur_M.faces))
        U.append(setup_deformation_transfer(M[-1],M[-2]))
