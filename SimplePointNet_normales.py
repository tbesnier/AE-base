...
class SimplePointNet(nn.Module):
    ...
    def __init__(self,
        ...
        self._conv_sizes = [6] + [k for k in conv_layer_sizes] + [self.CD]  # //2
    ...
    def forward(self, P, normales):
        '''
        Input: B x N x 3 point clouds (non-permuted)
        Output: B x LD embedded shapes
        '''
        #print(P.shape, normales.shape)
        P = torch.cat((P,normales),2)
        P = P.permute(0, 2, 1)
        assert P.shape[1] == 6, "Unexpected shape"
        # Now P is B x 3 x N
