...
class SimplePointNet(nn.Module):
    ...
    def __init__(self,
        ...
        self._conv_sizes = [4] + [k for k in conv_layer_sizes] + [self.CD]  # //2
    ...
    def forward(self, P, area):
        '''
        Input: B x N x 3 point clouds (non-permuted)
        Output: B x LD embedded shapes
        '''
        P = torch.cat((P, area), 2)
        P = P.permute(0, 2, 1)
        assert P.shape[1] == 4, "Unexpected shape"
        # Now P is B x 3 x N
...
class Decoder(nn.Module):
    def __init__(self, latent=512, filters=[1024, 2048], num_points=6890):
        ...
        for f_size in filters:
            ...
            size = f_size
        layers.append(nn.Linear(size, num_points*4))
